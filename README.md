...
[DUMUX](https://dumux.org/docs/doxygen/master/index.html) on VS Code.
...
```shell
code workspace-dumux/dumux.code-workspace
```
...
0. **Configure workspaces:** Collect test lists and creates `.vscode` folder
   using a python script.

1. **`dunecontrol` to configure:** Calls `dunecontrol` in the dumux installation
   folder (run in folder above module) to configure and build the module stack.

2. **`dunecontrol` to clean cache:** Calls `dunecontrol` in the dumux
   installation folder (run in folder above module) to clean the CMake cache.

3. **`duneproject` to create module:** Calls `duneproject` in the dumux
   installation folder (run in folder above module) to create a new module.

4. **`configure cmake`:** Reconfigure the module with `cmake build-cmake` (run
   in module folder).

5. **`make build_tests`:** Make all tests with `make build_tests` (run in
   `build-cmake` folder).

6. **Make specific test:** Make specific test with `make <test-name>` (Run in
   `build-cmake/{relative-file}` folder).

7. **Run test:** Run specific test with `<test-name> <param.input>`(Run in
   `build-cmake/{relative-file}` folder).
